...
//! real token counts. Wave 3 hardcodes the cap table; later waves can move it
//! to a config table without touching callers.
...
/// row exists yet, spent = 0 and calls = 0.
...
/// Debit the budget after a successful agent call. Upserts today's row.
pub async fn debit(
    pool: &PgPool,
    agent: &str,
    tokens_in: i64,
